...
async def delete_image(image_path: str):
    delete_image_path = os.path.join(images_dir, image_path)
    if os.path.exists(delete_image_path):
        os.remove(delete_image_path)
        return JSONResponse(content={"message": f"이미지 '{image_path}'가 삭제되었습니다."})
    else:
        raise HTTPException(status_code=404, detail="해당 경로에 이미지가 존재하지 않습니다.")
